(function () {
  ...
  var failed = false;
  // The `coworld-replay` bridge the platform embed listens on. `parent` is a
  // [Replaceable] attribute on Window, so viewer_smoke.mjs can install its own
  // receiver; the guard keeps a top-level open from talking to itself.
  function postReplayBridge(message) {
    try {
      if (window.parent === window) return;
      window.parent.postMessage(
        Object.assign({ src: 'coworld-replay' }, message), '*');
    } catch (ignored) { /* a cross-origin parent is not an error */ }
  }
  ...
  function showFailure(error) {
    ...
    // collab-cooking delta: the shell must SAY it failed. tools/ci/viewer_smoke.mjs
    // gates on data-replay-error, and the hosted embed reads the same attribute;
    // ctf only wrote #status, which is invisible to both.
    ...
    postReplayBridge({ type: 'error', message: message });
  }
  ...
  function createCore(config) {
    ...
    function onWorkerMessage(event) {
      ...
      try {
        if (message.type === 'text') {
          ...
          // collab-cooking delta: the `coworld-replay` bridge `ready` is posted
          // from INSIDE the branch that sets data-replay-loaded, never on rAF at
          // the call site -- otherwise softmax.com's embed samples an unpainted
          // shell and viewer_smoke.mjs, which accepts either signal, cannot see
          // the wrong order (chorus 3c11c953, 2026-08-24).
          postReplayBridge({ type: 'ready' });
          requestAnimationFrame(animate);
    ...
    function start() {
      ...
      try {
        worker = new Worker(workerUrl, { name: 'collab-cooking-static-replay' });
