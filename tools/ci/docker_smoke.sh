...
# Substitute: collab-cooking, coworld-collab-cooking, 4.
#
...
#
# env:
#   SMOKE_IMAGE                image, if not given as $1        (coworld-collab-cooking:ci)
#   SMOKE_SLUG                 game slug                        (collab-cooking)
#   SMOKE_GAME_BIN             game entrypoint                  (/bin/collab-cooking)
#   SMOKE_PLAYER_BIN           player entrypoint                (/bin/collab-cooking-player)
...
image="${1:-${SMOKE_IMAGE:-coworld-collab-cooking:ci}}"
slug="${SMOKE_SLUG:-collab-cooking}"
...
# Every PLAYER container must also have exited 0. Hosted certification checks
# this and reports `player_error`; a smoke that only checks the game's exit
# code passes a player that dies on a closed socket (raid 0.1.3 -> 0.1.4).
